...
# Load the wav file
audio = AudioSegment.from_wav("data/assessment_9.wav")
...
def split_audio(data):
    ...
    start_time = 0
    for item in data:
        end_time = int(item[0] * 1000)  # Convert to milliseconds
        audio_snippets.append(audio[start_time:end_time])
        start_time = end_time
    return audio_snippets

audio_snippets = split_audio(data)

def display_phoneme_and_audio(selected_phoneme):
    # Extract the actual index from the dictionary
    index = selected_phoneme["value"]
    # Display the phoneme and play the corresponding audio
    phoneme = processor.decode(data[index][1])
    play(audio_snippets[index])
    return phoneme
    
phonemes = [processor.decode(item[1]) for item in data]
phoneme_buttons = [{"label": phoneme, "value": i} for i, phoneme in enumerate(phonemes)]

gr.Interface(
    display_phoneme_and_audio, 
    gr.inputs.Radio(phoneme_buttons, label="Phonemes"), 
    gr.outputs.Textbox(label="Selected Phoneme")
).launch()
